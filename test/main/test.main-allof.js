...
module.exports = function testsAllOf() {
    it('should be function', function() {
        assert.equal(typeof jst.allOf, 'function');
    });
    ...
    describe('should return object when', function() {
        const test = (...args) => {
            const result = jst.allOf(...args);
            assert.equal(typeof result, 'object');
            assert.equal(has(result, 'allOf'), true);
            assert.equal(Array.isArray(result.allOf), true);
            return result;
        };
        ...
        it('call with Object', function() {
            ...
            assert.equal(result.allOf.length, 1);
            assert.equal(result.allOf[0], value);
        });

        it('call with Array', function() {
            const value = [{type: 'string'}, {type: 'number'}, {type: 'null'}];
            const result = test(value);
            assert.equal(result.allOf.length, 3);
            assert.equal(result.allOf[0], value[0]);
            assert.equal(result.allOf[1], value[1]);
            assert.equal(result.allOf[2], value[2]);
        });
        ...
        it('call with multiple arguments', function() {
            ...
            assert.equal(result.allOf.length, 6);
            assert.equal(result.allOf.includes(value[0]), true);
            assert.equal(result.allOf.includes(value[1]), true);
            assert.equal(result.allOf.includes(value[2]), true);
            assert.equal(result.allOf.includes(value1), true);
            assert.equal(result.allOf.includes(value2), true);
            assert.equal(result.allOf.includes(value3), true);
        });
    });
};
