...
module.exports = function testsOneOf() {
    it('should be function', function() {
        assert.equal(typeof jst.oneOf, 'function');
    });
    ...
    describe('should return object when', function() {
        const common = (...args) => {
            const result = jst.oneOf(...args);
            assert.equal(typeof result, 'object');
            assert.equal(has(result, 'oneOf'), true);
            assert.equal(Array.isArray(result.oneOf), true);
            return result;
        };
        ...
        it('call with Object', function() {
            ...
            assert.equal(result.oneOf.length, 1);
            assert.equal(result.oneOf[0], value);
        });
        ...
        it('call with Array', function() {
            ...
            assert.equal(result.oneOf[0], value[0]);
            assert.equal(result.oneOf[1], value[1]);
            assert.equal(result.oneOf[2], value[2]);
        });
        ...
        it('call with multiple arguments', function() {
            ...
            const result = common(value, value1, value2, value3);
            assert.equal(result.oneOf.length, 6);
            assert.equal(result.oneOf.includes(value[0]), true);
            assert.equal(result.oneOf.includes(value[1]), true);
            assert.equal(result.oneOf.includes(value[2]), true);
            assert.equal(result.oneOf.includes(value1), true);
            assert.equal(result.oneOf.includes(value2), true);
            assert.equal(result.oneOf.includes(value3), true);
        });
    });
};
