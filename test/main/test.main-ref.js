...
module.exports = function testsRef() {
    it('should be function', function() {
        assert.equal(typeof jst.ref, 'function');
    });
    ...
    describe('should return object when', function() {
        const test = (...args) => {
            const result = jst.ref(...args);
            assert.equal(typeof result, 'object');
            assert.equal(has(result, '$ref'), true);
            assert.equal(result.$ref, args[0]);
        };

        it(`call with '#/definitions/address'`, function() {
            test('#/definitions/address');
        });

        it(`call with 'definitions.json#/address'`, function() {
            test('definitions.json#/address');
        });
    });
};
