...
module.exports = function testsMultipleOf() {
    it('should be function', function() {
        assert.equal(typeof jstNumber.multipleOf, 'function');
    });

    it('should return class instance', function() {
        const result = jstNumber.multipleOf(1);
    ...
    describe('should throw error when', function() {
        const test = (...args) => {
            ...
            try {
                if (args.length > 0) {
                    jst.number().multipleOf(...args);
                } else {
                    jst.number().multipleOf();
                }
            } catch (err) {
                error = err;
            }
    ...
    describe('should create new property when', function() {
        const test = (...args) => {
            const result = jst.number().multipleOf(...args);
            assert.equal(typeof result, 'object');
