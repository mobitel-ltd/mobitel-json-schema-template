...
module.exports = function testsOneOf() {
    it('should be function', function() {
        assert.equal(typeof jstNumber.oneOf, 'function');
    });

    it('should return class instance', function() {
        const result = jstNumber.oneOf([{type: 'string'}]);
    ...
    describe('should create new property when', function() {
        const test = (...args) => {
            const result = jst.number().oneOf(...args);
            assert.equal(typeof result, 'object');
            assert.equal(has(result, 'rule'), true);
            assert.equal(has(result.rule, 'type'), true);
            assert.equal(has(result.rule, 'oneOf'), true);
            assert.equal(result.rule.type, 'number');
            assert.equal(Array.isArray(result.rule.oneOf), true);
            return result;
        };
        ...
        it('call with Object', function() {
            ...
            assert.equal(result.rule.oneOf.length, 1);
            assert.equal(result.rule.oneOf[0], value);
        });

        it('call with Array', function() {
            const value = [{type: 'string'}, {type: 'number'}, {type: 'null'}];
            const result = test(value);
            assert.equal(result.rule.oneOf.length, 3);
            assert.equal(result.rule.oneOf[0], value[0]);
            assert.equal(result.rule.oneOf[1], value[1]);
            assert.equal(result.rule.oneOf[2], value[2]);
        });

        it('call with multiple arguments', function() {
            const value = [{type: 'string'}, {type: 'number'}, {type: 'null'}];
            const value1 = {type: 'number'};
            const value2 = {type: 'boolean'};
            const value3 = {type: 'string'};
            const result = test(value, value1, value2, value3);
            assert.equal(result.rule.oneOf.length, 6);
            assert.equal(result.rule.oneOf.includes(value[0]), true);
            assert.equal(result.rule.oneOf.includes(value[1]), true);
            assert.equal(result.rule.oneOf.includes(value[2]), true);
            assert.equal(result.rule.oneOf.includes(value1), true);
            assert.equal(result.rule.oneOf.includes(value2), true);
            assert.equal(result.rule.oneOf.includes(value3), true);
        });
        ...
        it('multiple calls, set property value from last call', function() {
            ...
            const result = jst.number()
                ...
                .oneOf(value2)
                .oneOf(value3);
            assert.equal(result.rule.oneOf[0], value3[0]);
            assert.equal(result.rule.oneOf[1], value3[1]);
        });
    });
};
