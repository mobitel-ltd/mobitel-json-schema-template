...
module.exports = function testsAllOf() {
    it('should be function', function() {
        assert.equal(typeof jstNumber.allOf, 'function');
    });

    it('should return class instance', function() {
        const result = jstNumber.allOf([{type: 'string'}]);
    ...
    describe('should create new property when', function() {
        const test = (...args) => {
            const result = jst.number().allOf(...args);
            assert.equal(typeof result, 'object');
            assert.equal(has(result, 'rule'), true);
            assert.equal(has(result.rule, 'type'), true);
            assert.equal(has(result.rule, 'allOf'), true);
            assert.equal(result.rule.type, 'number');
            assert.equal(Array.isArray(result.rule.allOf), true);
            return result;
        };
        ...
        it('call with Object', function() {
            ...
            assert.equal(result.rule.allOf.length, 1);
            assert.equal(result.rule.allOf[0], value);
        });

        it('call with Array', function() {
            const value = [{type: 'string'}, {type: 'number'}, {type: 'null'}];
            const result = test(value);
            assert.equal(result.rule.allOf.length, 3);
            assert.equal(result.rule.allOf[0], value[0]);
            assert.equal(result.rule.allOf[1], value[1]);
            assert.equal(result.rule.allOf[2], value[2]);
        });
        ...
        it('call with multiple arguments', function() {
            ...
            assert.equal(result.rule.allOf.length, 6);
            assert.equal(result.rule.allOf.includes(value[0]), true);
            assert.equal(result.rule.allOf.includes(value[1]), true);
            assert.equal(result.rule.allOf.includes(value[2]), true);
            assert.equal(result.rule.allOf.includes(value1), true);
            assert.equal(result.rule.allOf.includes(value2), true);
            assert.equal(result.rule.allOf.includes(value3), true);
        });
        ...
        it('multiple calls, set property value from last call', function() {
            ...
            const result = jst.number()
                ...
                .allOf(value2)
                .allOf(value3);
            assert.equal(result.rule.allOf[0], value3[0]);
            assert.equal(result.rule.allOf[1], value3[1]);
        });
    });
};
