...
module.exports = function testsMultipleOf() {
    it('should be function', function() {
        assert.equal(typeof jstInteger.multipleOf, 'function');
    });

    it('should return class instance', function() {
        const result = jstInteger.multipleOf(1);
    ...
    describe('should throw error when', function() {
        const test = (...args) => {
            ...
            try {
                if (args.length > 0) {
                    jst.integer().multipleOf(...args);
                } else {
                    jst.integer().multipleOf();
                }
            } catch (err) {
                error = err;
            }
    ...
    describe('should create new property when', function() {
        const test = (...args) => {
            const result = jst.integer().multipleOf(...args);
            assert.equal(typeof result, 'object');
            assert.equal(has(result, 'rule'), true);
            assert.equal(has(result.rule, 'type'), true);
            assert.equal(has(result.rule, 'multipleOf'), true);
            assert.equal(result.rule.type, 'integer');
            assert.equal(result.rule.multipleOf, args[0]);
        };
        ...
        it('multiple calls, set property value from last call', function() {
            const result = jst.integer()
                ...
                .multipleOf(2)
                .multipleOf(3);
            assert.equal(result.rule.multipleOf, 3);
        });
    });
};
